...
export function SiteHeader() {
  ...
  return (
    <header className="site-header">
      ...
      <AnimatePresence>{open && <motion.div className="mobile-drawer" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: .38, ease: [0.22, 1, 0.36, 1] }}><span>NAVIGATION / 00</span><nav><Link onClick={() => setOpen(false)} href="/how-it-works"><T>How it works</T> <b>01</b></Link><Link onClick={() => setOpen(false)} href="/principles"><T>Principles</T> <b>02</b></Link><Link onClick={() => setOpen(false)} href="/plan"><T>Outreach planner</T> <b>03</b></Link><Link onClick={() => setOpen(false)} href="/pacing"><T>Portfolio pacing</T> <b>04</b></Link><Link onClick={() => setOpen(false)} href="/workspace"><T>Founder workspace</T> <b>05</b></Link><Link onClick={() => setOpen(false)} href="/catalogue"><T>VC catalogue</T> <b>06</b></Link><Link onClick={() => setOpen(false)} href="/open-source"><T>Open source</T> <b>07</b></Link></nav><LanguageSwitcher className="mobile" /><Link className="button button-accent" onClick={() => setOpen(false)} href="/signup"><T>Join FirstContact</T> ↗</Link></motion.div>}</AnimatePresence>
    </header>
  );
}
